...
// Loading steps for RAG processing animation
const LOADING_STEPS = [
...
// Helper component for Gemini-style loading indicator (single step, fades between)
const LoadingIndicator = ({ currentStep }: { currentStep: string }) => {
  const step =
    LOADING_STEPS.find((s) => s.key === currentStep) || LOADING_STEPS[0];

  return (
    <div className="flex items-center gap-2">
      <span
        className="text-sm text-gray-500 italic animate-pulse transition-opacity duration-300"
        key={step.key}
      >
        {step.label}
...
export default function ChatBot() {
  ...
  // Helper to animate through loading steps
  ...
  // Reset loading step when loading completes
  useEffect(() => {
    if (!isLoading) {
      setLoadingStep("thinking");
    }
  ...
  const handleQuickReply = async (payload: string, title: string) => {
    ...
    try {
      ...
      const stillOnboarding =
        !response.completed && response.status === "onboarding";
      ...
      if (!stillOnboarding) {
        const botMessage: Message = {
          ...
          text:
            response.message ||
            "Great! I'm ready to help with your tax questions. Ask me anything!",
          sender: "bot",
          timestamp: new Date(),
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      // PRIORITY 3: For ALL other typed messages (including during onboarding), use RAG
      // Rationale: Onboarding should ONLY advance via button clicks (handleQuickReply),
      // not via typed text. If user types "Hi" during onboarding, they want to chat,
      // not respond to the consent question.
      else {
        try {
          // Get user context if available
          const userStr = localStorage.getItem("user");
            ...
            {messages.map((message) => (
              ...
              >
                ...
                <div className="max-w-[75%] flex flex-col gap-1">
                  ...
                  >
                    {/* Thought process removed - cleaner UI */}
                      ...
                      <div className="flex flex-col gap-2 mt-1">
                        {message.quickReplies.map((reply) => (
                          <Button
                            ...
                            size="sm"
                            onClick={() =>
                              handleQuickReply(reply.payload, reply.title)
                            }
                            disabled={isLoading}
            ...
            {isLoading && messages.length > 0 && (
              <div className="flex justify-start gap-2">
                ...
                <div className="bg-white p-3 rounded-lg shadow-sm min-w-[200px]">
                  <LoadingIndicator currentStep={loadingStep} />
                </div>
              </div>
            )}
